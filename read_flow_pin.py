...
for loop_counter in range(0, 50000000):
    ...
    if current_value != last_value:
        ...
        if transition_count % 2 == 0:
            if (time.time() != time_window_begin):
                print ("Loop counter: ", loop_counter)
                nowish = time.time()
                ...
                est_flow_rate_window_total = (60.0/(nowish - time_window_begin)) * transition_count * (volume_multipler / 2.0)
                total_flow_to_date = transition_count * volume_multiplier / 2.0
                print ("Estimated flow rate (gal/min): ", est_flow_rate)
                ...
                print ("Current value: ", current_value)
                time_window_short_beging = nowish
    time.sleep(0.25) 
